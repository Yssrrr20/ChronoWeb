...
const ProfileIcon = () => <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 0 00-7-7z" /></svg>;

const menuItems = [
    // Pastikan URL sesuai dengan struktur folder Anda di app router
    { name: 'Home', href: '/Dashboard', icon: <HomeIcon /> }, // Jika dashboard adalah home
    ...
    <div className="hidden md:flex flex-col w-64 p-4 bg-white dark:bg-gray-800 border-r dark:border-gray-700 flex-shrink-0">
      <nav className="flex-grow pt-4">
        <ul className="space-y-1">
          {menuItems.map((item) => (
            <li key={item.name}>
              {/* NavItem akan menangani Link dan penandaan aktif */}
              <UserNavItem href={item.href} icon={item.icon} text={item.name} />
            </li>
          ))}
        </ul>
      </nav>
